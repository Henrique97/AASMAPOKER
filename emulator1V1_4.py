...
emulator = Emulator()
#quuid = "uuid-q"
p_uuid=["high-iq","low-iq","low-iq2","uuid-4"]
...
for i in range(0, 60):
	...
	emulator.register_player(uuid=p_uuid[1], player=HonestPlayer(n_players,10))
	emulator.register_player(uuid=p_uuid[2], player=HonestPlayer(n_players,10))
	emulator.set_game_rule(player_num=3, max_round=1000, small_blind_amount=20, ante_amount=0)
	# 2. Setup GameState object
	players_info = {
		p_uuid[0]: { "name": "player1", "stack": 100000 },
		p_uuid[1]: { "name": "player2", "stack": 100000 }
		p_uuid[2]: { "name": "player3", "stack": 100000 }
	}
	...
	df1.to_csv("job4/4gameAllroundsP1" + str(i) + ".csv")
	df2.to_csv("job4/4gameAllroundsP2"+ str(i) + ".csv")
	df3.to_csv("job4/4gameAllroundsP3"+ str(i) + ".csv")

df.to_csv("job4/4gameresults.csv")
